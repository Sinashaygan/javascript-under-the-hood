...
# Program Performance — Cheat Sheet

Async ≠ Parallel

Async programming:
- Improves concurrency
- Helps independent operations overlap
- Does not automatically move CPU work to another thread

Web Worker:
- Runs JavaScript in a separate thread
- Keeps CPU-heavy work away from the main UI thread
...
Shared Worker:
- Multiple pages/tabs ↔ one Worker
- Uses MessagePorts
...
Data Parallelism
→ SIMD
→ Same operation over multiple data values

asm.js:
- Optimizable subset/style of JavaScript
- Focuses on predictable types and low-level optimization
- Often generated by compilers
- Not normally written manually

Main takeaway:

Asynchrony helps manage concurrency,
while Workers can provide actual parallel execution
